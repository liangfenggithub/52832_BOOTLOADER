...
#define BOOTLOADER_REGION_START              0x00076000//0x0007A000                  /**< This field should correspond to start address of the bootloader, found in UICR.RESERVED, 0x10001014, register. This value is used for sanity check, so the bootloader will fail immediately if this value differs from runtime value. The value is used to determine max application size for updating. */
...
#define DFU_APP_DATA_RESERVED           CODE_PAGE_SIZE * 2//0       //保留用户配置，bootloader起始地址是  0x76000 程序配置在0x74000开始存储，那么该值为 0x76000-0x74000 = 0x2000/CODE_PAGE_SIZE = 2                                    /**< Size of Application Data that must be preserved between application updates. This value must be a multiple of page size. Page size is 0x400 (1024d) bytes, thus this value must be 0x0000, 0x0400, 0x0800, 0x0C00, 0x1000, etc. */
#endif
